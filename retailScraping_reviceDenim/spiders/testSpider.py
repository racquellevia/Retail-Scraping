...
class MySpider(CrawlSpider):
    name = "reviceCrawl"
    allowed_domains: ["revicedenim.com"]
    start_urls = ["https://www.revicedenim.com/collections/everything?page=1"]
...
#        price = response.css('.money::text').extract()
        link = response.css("a.ProductItem__ImageWrapper").xpath('@href').extract()
        linkFull =[]
        for lnk in link:
...
#            yield scraped_info
    ...
    def parse_link(self, response):
        ...
        name = response.css(".ProductMeta__Title.Heading.u-h2::text").extract()
